...
def test_answer_question_no_documents_returns_message():
    with patch.object(rag_chain, "similarity_search", return_value=[]):
        result = rag_chain.answer_question("Wie hoch ist die Leasingrate?")
    assert result["sources"] == []
    assert "keine Dokumente" in result["answer"]
...
def test_answer_question_with_sources():
    ...
    fake_response.content = "Die monatliche Leasingrate beträgt 389,00 EUR [Quelle 1]."
    ...
    assert result["sources"][0]["marker"] == "Quelle 1"
